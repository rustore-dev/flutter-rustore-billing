...
data class PurchaseAvailabilityResultFlutter (
  ...
  companion object {
    ...
    fun fromList(list: List<Any?>): PurchaseAvailabilityResultFlutter {
      ...
      val unavailableCause = list[2] as String?
...
interface RustoreBilling {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: RustoreBilling?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.flutter_rustore_billing.RustoreBilling.getAuthorizationStatus", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            api.getAuthorizationStatus() { result: Result<Boolean> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
